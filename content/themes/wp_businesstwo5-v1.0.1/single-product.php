...
<article class="row">
	...
	<div class="twelve columns">
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('entry group'); ?>>
			...
			<?php else : //product slider not enabled ?>
...
data-fancybox-group="p['.get_the_ID().']" title="'.esc_attr($alt_text).'">'.wp_get_attachment_image( $attachment->ID, 'ci_portfolio_list_big', false, $attr ).'</a></li>';
		 ...
		 <?php else : ?>
		 <img class="scale-with-grid entry-thumb" src="<?php echo get_template_directory_uri(); ?>/images/image_thumb_not_avail.jpg" alt="">
	...
	<div id="sidebar" class="four columns">
		<h3 class="widget-title"><?php _e('About this Product', 'ci_theme'); ?></h3>
		<p><?php the_content(); ?></p>

		<?php
		$project_fields = get_post_meta(get_the_ID(), 'ci_cpt_project_info_fields', true);
		$work_url = get_post_meta(get_the_ID(), 'ci_cpt_product_url', true);
		$content_cols = 'ten';
		...
		<div id="work-desc">
			<h3 class="widget-title"><?php _e('Product Info', 'ci_theme'); ?></h3>
			<ul class="work-credits">
				...
				<li><a href="<?php echo $work_url; ?>" class="btn visit-btn"><?php _e("product link", 'ci_theme'); ?></a></li>
				<?php endif; ?>
			</ul>
		</div> <!-- #work-desc -->
		<?php endif; ?>
	...
	<?php if ( !ci_setting('disable_product_related') ) : ?>
	<?php
	// Related Work Query
	$term_list = array();
